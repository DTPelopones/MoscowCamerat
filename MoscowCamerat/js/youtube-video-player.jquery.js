...
(function ($) {
    $.youtube_video = function (el, options) {
                ...
                if (typeof (yt.items) !== 'undefined') {
                    ...
                    if (base.options.pagination === true) {
                        if (typeof (yt.nextPageToken) === typeof (undefined)) {
                            ...
                            base.$el.find('.yesp-next-page').html('<i class="yesp-icon yesp-icon-plus material-icons"></i><span class="load-more-label">' + base.options.load_more_text + '</span>').show();
                        }
        ...
        base.create_playlist = function (through_pagination, items, count) {
            ...
            for (base.info.playlist_i; base.info.playlist_i < base.playlist_count + count; base.info.playlist_i++) {
                ...
                var $video_hp = $('<div class="yesp-hp-video" data-playing="0" data-index="' + base.info.playlist_i + '"><img src="' + img_src + '" width="200" /><div class="yesp-hp-overlay"><div class="yesp-hp-title">' + video_title + '</div><div class="yesp-hp-channel">' + video.snippet.channelTitle + '</div></div><div class="yesp-hp-current"><i class="yesp-icon yesp-icon-play material-icons"></i><span class="current-play-label">' + base.options.now_playing_text + '</span></div></div>');
        ...
        base.create_player_element = function () {
            base.$el.css('width', '100%').addClass('yesp').html('<div class="yesp-container"><div class="yesp-autoposter"><div class="yesp-autoposter-icon"><div></div></div></div><div class="yesp-video-container"><div class="yesp-video" id="' + base.id + '_yt"></div><div class="yesp-error"></div></div></div><div class="yesp-playlist"><div class="yesp-next-page"><i class="yesp-icon yesp-icon-plus material-icons"></i>Load More</div></div><div class="yesp-hp"><div class="yesp-hp-videos"><div class="yesp-hp-next-page"><i class="yesp-icon yesp-icon-plus material-icons"></i></div></div></div>');
        ...
        base.create_controls = function () {
            ...
            /* ded - add custom title */
            var volume_title = (base.info.volume) ? base.options.mute_off_title : base.options.mute_on_title;
            $controls.html(
				'<div class="yesp-controls-wrapper">'
				+ '<a href="#" class="yesp-play yesp-icon yesp-icon-play"></a>'
				+ '<div class="yesp-time">00:00 / 00:00</div>'
				+ '<div class="yesp-bar"><div class="yesp-bar-buffer"></div><div class="yesp-bar-time"></div></div>'
				+ '<div class="yesp-volume"><a href="#" class="yesp-volume-icon yesp-icon yesp-icon-volume-up" data-title="' + volume_title + '"></a>'
				+ '<div class="yesp-volume-bar"><div class="yesp-volume-amount"></div></div></div>'
				+ '<a href="#" class="yesp-share yesp-icon yesp-icon-share-square-o" title="' + base.options.share_title + '"></a>'
				+ '<a href="#" target="_blank" class="yesp-youtube yesp-icon yesp-icon-youtube-play" title="' + base.options.open_in_youtube_title + '"></a>'
				+ '<a href="#" class="yesp-backward yesp-icon yesp-icon-backward" title="' + base.options.prev_video_title + '"></a>'
				+ '<a href="#" class="yesp-forward yesp-icon yesp-icon-forward" title="' + base.options.next_video_title + '"></a>'
				+ '<a href="#" class="yesp-playlist-toggle yesp-icon yesp-icon-align-justify yesp-icon-rotate-180" data-title="' + base.options.hide_playlist_title + '"></a>'
				+ '<a href="#" class="yesp-fullscreen yesp-icon yesp-icon-expand" data-title="' + base.options.enter_fullscreen_title + '"></a></div>'
			);
        ...
        base.play_video = function (index, cue, fast_scroll) {
            ...
            base.get_video_duration(video.snippet.resourceId.videoId);

        };

        // get first_video duration - ded
        base.get_video_duration = function (video_id) {
            var url = 'https://www.googleapis.com/youtube/v3/videos?id=' + video_id + '&part=contentDetails&key=' + base.options.api_key,
				r = $.getJSON(url, function (resp) {
				    var d = base.format_video_duration(resp.items[0].contentDetails.duration);
				    base.$controls['time'].text('00:00 / ' + d);
				});
        ...
        //@param duration is an ISO_8601 duration - ded
        base.format_video_duration = function (duration) {
            var sec = 0;
            if (duration.match(/(\d+)D/)) {
                sec += duration.match(/(\d+)D/)[1] * 24 * 3600;
            }
            if (duration.match(/(\d+)H/)) {
                sec += duration.match(/(\d+)H/)[1] * 3600;
            }
            if (duration.match(/(\d+)M/)) {
                sec += duration.match(/(\d+)M/)[1] * 60;
            }
            if (duration.match(/(\d+)S/)) {
                sec += duration.match(/(\d+)S/)[1] - 1;
            }
            return base.format_time(sec);
        };
        ...
        base.enter_fullscreen = function () {
            ...
            base.$el.find('.yesp-icon-expand').removeClass('yesp-icon-expand').addClass('yesp-icon-expand-out').data('title', base.options.exit_fullscreen_title); // ded
            base.$el.find('.yesp-playlist-toggle').css('display', 'none'); // ded
        };
        ...
        base.exit_fullscreen = function (exit) {
            ...
            base.resize();
            base.$el.find('.yesp-icon-expand-out').removeClass('yesp-icon-expand-out').addClass('yesp-icon-expand').data('title', base.options.enter_fullscreen_title); // ded
            base.$el.find('.yesp-playlist-toggle').css('display', 'block'); // ded
            // base.show_controls();
        };
        ...
        base.show_playlist = function (fast, resize) {
            ...
            base.$el.find('.yesp-icon-list').removeClass('yesp-icon-list').addClass('yesp-icon-align-justify').data('title', base.options.hide_playlist_title); // ded
        ...
        base.hide_playlist = function (fast, resize) {
            ...
            if (base.options.playlist_type == 'vertical') {
                base.$el.find('.yesp-icon-align-justify').removeClass('yesp-icon-align-justify').addClass('yesp-icon-list').data('title', base.options.show_playlist_title); // ded
            }
        ...
        base.update_volume = function (x, vol) {
            ...
            if (percentage == 0) {
                base.$controls['volume_icon'].addClass('yesp-icon-volume-off').removeClass('yesp-icon-volume-up').data('title', base.options.mute_off_title);
            } else {
                var vol = base.$controls['volume_icon'];
                vol.removeClass('yesp-icon-volume-off').addClass('yesp-icon-volume-up').data('title', base.options.mute_on_title);
                if (percentage < 50) {
                    vol.addClass('material-volume-down');
                } else {
                    vol.removeClass('material-volume-down');
                }
            }
        ...
        base.format_time = function (seconds) {
			...
			//m = Math.floor((seconds - (h * 3600)) / 60) < 10 ? "0" + Math.floor(seconds / 60) : Math.floor(seconds / 60),
			//s = Math.floor(seconds - (m * 60)) < 10 ? "0" + Math.floor(seconds - (m * 60)) : Math.floor(seconds - (m * 60));
				m = Math.floor((seconds - (h * 3600)) / 60) < 10 ? "0" + Math.floor((seconds - (h * 3600)) / 60) : Math.floor((seconds - (h * 3600)) / 60),
				s = Math.floor(seconds - (h * 3600 + m * 60)) < 10 ? "0" + Math.floor(seconds - (h * 3600 + m * 60)) : Math.floor(seconds - (h * 3600 + m * 60));
            if (h != 0) {
                return h + ":" + m + ":" + s;
            }
            return m + ":" + s;
    ...
    $.youtube_video.defaultOptions = {
        ...
        colors: {},
        /* ded - add options */
        mute_off_title: 'Mute Off',
        mute_on_title: 'Mute On',
        share_title: 'Share',
        open_in_youtube_title: 'Open in YouTube',
        prev_video_title: 'Previous Video',
        next_video_title: 'Next Video',
        hide_playlist_title: 'Hide Playlist',
        show_playlist_title: 'Show Playlist',
        enter_fullscreen_title: 'Enter Fullscreen',
